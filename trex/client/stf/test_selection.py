            ...
            while test['status'] and task.test_count < task.max_test_count and task.succ_attempt < (task.max_succ_attempt + 1) and task.rate > 0:
                ...
                if not drop:
                    ...
                    if task.type == 'safe':
                        ...
                        for drop_count in test['values']['sampler']:
                            if int(drop_count['rx_drop_bps']) > 0:
                                drop = True
                                break
                        if (int(tx_p_0 - rx_p_1) <= int(tx_p_0 * task.accuracy)) and (int(tx_p_1 - rx_p_0) <= int(tx_p_1 * task.accuracy)) and not drop:
                            test_succ = True
                    # for accurancy test type
                        ...
                        tx_p_1 = test['values']['global']['tx_ptks']['opackets-1']
                        rx_p_0 = test['values']['global']['rx_ptks']['ipackets-0']
                        if (int(tx_p_0 - rx_p_1) <= int(tx_p_0 * task.accuracy)) and (int(tx_p_1 - rx_p_0) <= int(tx_p_1 * task.accuracy)):
                            test_succ = True
                    # for drop test type
                    elif task.type == 'drop':
                        for drop_count in test['values']['sampler']:
                            if int(drop_count['rx_drop_bps']) > (int(drop_count['rx_bps']) * task.accuracy):
                                drop = True
                                break
                        if not drop:
                            test_succ = True
